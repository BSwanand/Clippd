...
describe("Validating deleting an activity", () => {
    ...
    it.skip("Delete an activity and verifying the success message", () => {
        ...
        utils.getElement(activitiesPage.acitivitybeingDeletedToastMsg).should('be.visible');

        //Verifying if activity was deleted succesfully toast message is visible
        utils.getElement(activitiesPage.activityWasDeletedSuccesFullyToastMsg).should('be.visible');
...
describe("Validating editing an activity", () => {
    ...
    it.skip("Edting an activity and verifying the success message", () => {
        ...
        //Editing the title of activity with some ranodm text
        var randomActivityName = activitiesPage.editActivityTitleWithSomeRandomText();

        //Saving the activity
        utils.clickOn(activitiesPage.saveActivityBtn);

        //Verifying if tilte updated succesfully text toast is displayed
        utils.getElement(activitiesPage.activityTitleUpdatedSuccesfullyToast).should('be.visible');

        //Verifying if the activity name has been changed to ranodm activity name
        utils.getText(activitiesPage.activityTitlelabel).should('eq', randomActivityName);
    ...
    it("Edit Select Tee on Edit Activity Screen", () => {
        ...
        activitiesPage.selectRandomAutoSuggestion().then(($suggestions) => {

            cy.wrap(Cypress._.sampleSize($suggestions.toArray(), 1)).click();
        }).wait(5000)
            .then(function () {
                utils.getText(activitiesPage.selectedTeeValue).as('teeColorAfterSelecting')

                utils.clickOn(activitiesPage.saveDetailsBtn);

                utils.getElement(activitiesPage.activiityIsBeingUpdated).should('not.be.visible');
        ...
        cy.wait(20000).then(function () {
            utils.getText(activitiesPage.teeColourOnFirstActiviyRow).then(function (teeColorOnFirstActiviyRow) {
                expect(this.teeColorAfterSelecting).to.contain(teeColorOnFirstActiviyRow);
            })
    ...
    it("Edit Date and Time on Edit Activity Screen", () => {
        ...
        cy.wait(5000).then(() => {
            ...
            var time = Cypress._.sampleSize(['12:30 PM', '11:30 AM', '4:30 PM', '5:30 PM', '3:00 PM', '7:00 PM'], 1)
            ...
            cy.wait(5000).then(() => {
                cy.xpath(activitiesPage.selectedDateOnCalendar).each((item, index, list) => {
            ...
            utils.waitFor(30000);
            utils.getText(activitiesPage.dateOnFirstActivitylink).then(function (DateOnFirstRow) {

                var SelectedDateOnCal = this.randomDay + " " + this.randomMonth + " " + this.randomYear;
    ...
    it('Edit Weather and course conditions on Edit Activity Screen', function () {
        ...
        activitiesPage.selectOptionFromDropdown(windvalue).should('be.visible').click(
            {
                force: true
            })
        cy.wait(2000)
        utils.clickOn(activitiesPage.rainDropdown)
        activitiesPage.selectOptionFromDropdown(rainvalue).should('be.visible').click(
            {
                force: true
            })
        cy.wait(2000)
        utils.clickOn(activitiesPage.fairWayDropdown)
        activitiesPage.selectOptionFromDropdown(fairwayValue).should('be.visible').click(
            {
                force: true
            })
        cy.wait(2000)
        utils.clickOn(activitiesPage.roughDropdown)
        activitiesPage.selectOptionFromDropdown(roughValue).should('be.visible').click(
            {
                force: true
            })
        cy.wait(2000)
        utils.clickOn(activitiesPage.greenSpeed)
        activitiesPage.selectOptionFromDropdown(greenSpeed).should('be.visible').click(
            {
                force: true
            })
        cy.wait(5000)
        ...
        cy.xpath(activitiesPage.activityHasBeenUpdated,
            ...
            .then(function () {
                ...
                cy.wait(5000);
                var arr = utils.getTextOfAllElements("//*[contains(@id,'tippy')]/div/div/p");
                cy.wrap(arr).then(function (tooltipArray) {
                    let selectedOptionsArray = ['Rain:' + " " + this.selectedrainvalue, 'Wind:' + " " + this.selectedwindvalue, 'Fairway:' + " " + this.selectedfairwayValue, 'Rough:' + " " + this.selectedroughValue, 'Greens:' + " " + this.selectedgreenSpeed];
    ...
    it('Verify Course search on Edit Activity Screen', function () {
        ...
        var autoSuggestions = utils.getTextOfAllElements(activitiesPage.courseNameAutosuggestions);
    ...
    it('verifying if after changing course name the deatils are updated', function () {
        ...
        var selectedCourseNameVal = Cypress._.sampleSize(['Sunningdale GC - Old', 'Frilford Heath GC - Blue', 'Gog Magog GC - Wandlebury', 'Birkdale GC, Huntersville', 'New York CC, New Hempstead'], 1)

        cy.wrap(selectedCourseNameVal).as('selectedCourseName').then(function () {
            activitiesPage.searchCourseName(this.selectedCourseName + '');
            cy.get(activitiesPage.searchCourseNameInput).type('{downArrow}').type('{enter}');
            ...
            utils.getText(activitiesPage.courseNameOnFirstRow).then(function (courseNameOn1stRow) {
                cy.log(courseNameOn1stRow);

                expect(this.selectedCourseName + ''.trim()).to.include(courseNameOn1stRow.split("-").pop().trim());
    ...
    it('Verifting Refresh shots from data source feature', function () {
        ...
        utils.getElement(activitiesPage.activiityIsBeingUpdated).should('be.visible')
