...
describe("validating off the tee testcases", function () {
    ...
    it("Verifying if ott shot quality contains only numbers not text", function () {
        utils.getElement(offTheTeePage.ottShotQualityValues)
            .filter(':first')
            .then(($value) => {
                cy.wrap($value.text().trim()).should('match', /^[0-9]*$/)
            })


    })

    it.skip("verifying if fw hit is having % sign at the end and contains only numbers", function () {
    ...
    it.skip("Veryfying What to Focus on section - Labels", function () {
        //verifying if the label is visible
    ...
    it.skip("Verifying if negative impact section is having red color", function () {
