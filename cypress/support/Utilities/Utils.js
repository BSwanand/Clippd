...
class Utils {
    ...
    clickOn(element_xpath_or_css) {
        if (element_xpath_or_css.startsWith('/') || element_xpath_or_css.startsWith('(')) {
    ...
    waitTillPageLoad() {
        cy.window().then((win) => {
            ...
            cy.xpath("//button[@type='submit' and text()='Log in']", { timeout: 60000 }, { log: false }).should('not.exist');
        });
    }
    ...
    extractnumber(input) {
        var regex = /[+-]?\d+(\.\d+)?/g;
        var str = input;
        var finalans = str.match(regex).map(function (v) { return parseFloat(v); });
